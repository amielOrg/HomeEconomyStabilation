...
| `tests/e2e/credit-card-upload.e2e.spec.ts` | Real workbook import across CSV, SpreadsheetML, .xlsx with and without cell references, windows-1255 encoding and English column names, multi-file imports, localized unrecognised-layout reporting, duplicate re-imports, header labels after import, evidence-based transfer/alimony categorization, leisure, studies, clothing, taxes, Jewish life and giving each separated from the bills, salary, shops and transfers they arrive beside, every added category offered in every language, honest unknown fallback, upload availability and recommendations |
