...
| `tests/e2e/credit-card-upload.e2e.spec.ts` | Real workbook import across CSV, SpreadsheetML, .xlsx with and without cell references, windows-1255 encoding and English column names, multi-file imports, localized unrecognised-layout reporting, duplicate re-imports, header labels after import, evidence-based transfer/alimony categorization, leisure separated from the household bills it arrives beside and offered in every language, studies and clothing separated from the salary and shops they arrive beside and offered in every language, honest unknown fallback, upload availability and recommendations |
